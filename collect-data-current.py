...
while True:
    ...
    if left_handed == True:
        ...
        roi1_dict = {
            'Name' : 'roi',
            'Object': frame[y1_one:y2_one,x1_one:x2_one],
            'Coords': {
                'x1': 10,
                'y1': int(frameheight / 6),
                'x2': int(framewidth / 4),
                'y2': int(2*frameheight / 3),
                'vertices' :  ((x1_one-1, y1_one-1), (x2_one+1, y2_one+1))
        ...
        roi1_dict = {
            ...
            'Object': frame[y1_one:y2_one,x1_one:x2_one],
            'Coords': {
                'x1': int(3*framewidth / 4),
                'y1': int(frameheight / 6),
                'x2': int(framewidth - 10),
                'y2': int(2*frameheight / 3),
                'vertices' :  ((x1_one-1, y1_one-1), (x2_one+1, y2_one+1))
    ...
    roi2_dict = {
        'Name' : 'roi',
        'Object': frame[y1_two:y2_two,x1_two:x2_two],
        'Coords': {
            'x1': int(framewidth / 4),
            'y1': int(2*frameheight / 4),
            'x2': int(3*framewidth / 4),
            'y2': int(frameheight - 10),
            'vertices' :  ((x1_two-1, y1_two-1), (x2_two+1, y2_two+1))
    ...
    available_keys = list(map(str, (ord('0'), ord('1'), ord('2'), ord('3'), ord('4'), ord('5'))))
    last_key = k
    k = cv2.waitKey(100)
    ...
    if str(k) in available_keys :
        if last_key == ord('s') :
            if k == ord('0') or k == ord('1'):
                auto_cap(k, frame, roi2_dict)
            else:
                auto_cap(k, frame, roi1_dict)
