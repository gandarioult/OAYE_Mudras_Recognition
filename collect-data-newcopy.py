...
while True:
    ...
    framecenter = (int(0.5*framewidth), int(0.5*frameheight))
    textplaceholder1 = (10, framecenter[1]+30)
    ...
    if left_handed == True:
        x1_one = 10
        y1_one = int(frameheight - 10)
        x2_one = int(framecenter[0] / 2)
        y2_one = 10
        rect_coords1 = ((x1_one-1, y1_one-1), (x2_one+1, y2_one+1))
        #cv2.rectangle(frame, rect_coords1[0], rect_coords1[1], (255,0,0) ,1)
        cv2.rectangle(frame, (x1_one,y1_one), (x2_one,y2_one), (255,0,0))

    elif right_handed == True:
        x1_one = int(framecenter[0] * 1.5)
        y1_one = int(frameheight - 10)
    ...
    #2handed ROI
    if center == True:
        x1_two = int(framecenter[0] / 2)
        y1_two = int(framecenter[1] / 2)
        x2_two = int(framecenter[0] * 1.5)
        y2_two = 10
        rect_coords2 = ((x1_two-1, y1_two-1), (x2_two+1, y2_two+1))
        #cv2.rectangle(frame, rect_coords2[0], rect_coords2[1], (255,0,0) ,1)
        cv2.rectangle(frame, (x1_two,y1_two), (x2_two,y2_two), (255,0,0))
    else:
        pass
    ...
    if k == ord('0') and last_key == ord('s') and center is True: 
        ...
        while kp == -1 :        ###LOOP    
            ...
            cv2.rectangle(frame, rect_coords2[0], rect_coords2[1], (255,0,0) ,1)  ##extracting the ROI
            roi2 = frame[y1_two:y2_two, x1_two:x2_two]
